...
class GenericNtupleMakerSNU8TeV : public edm::EDAnalyzer
{
public:
  GenericNtupleMakerSNU8TeV(const edm::ParameterSet& pset);
...
GenericNtupleMakerSNU8TeV::GenericNtupleMakerSNU8TeV(const edm::ParameterSet& pset)
{
...
void GenericNtupleMakerSNU8TeV::analyze(const edm::Event& event, const edm::EventSetup& eventSetup)
{
  ...
  for (auto jt : *jets) {
    ...
    std::cout << "Jet " << jt.pt()    <<  " " << jt.eta() << " " << jt.bDiscriminator("combinedSecondaryVertexBJetTags") << std::endl;
  }  
  ...
  {
    edm::LogError("GenericNtupleMakerSNU8TeV") << "Failed to get " << nFailure << " items";
...
void GenericNtupleMakerSNU8TeV::endLuminosityBlock(const edm::LuminosityBlock& lumi, const edm::EventSetup& eventSetup)
{
...
DEFINE_FWK_MODULE(GenericNtupleMakerSNU8TeV);
