...
class GenericNtupleMakerSNU8TeVPAT : public edm::EDAnalyzer
{
public:
  GenericNtupleMakerSNU8TeVPAT(const edm::ParameterSet& pset);
...
GenericNtupleMakerSNU8TeVPAT::GenericNtupleMakerSNU8TeVPAT(const edm::ParameterSet& pset)
{
...
void GenericNtupleMakerSNU8TeVPAT::analyze(const edm::Event& event, const edm::EventSetup& eventSetup)
{
  ...
  event.getByToken(src_, src);
  std::cout << event.id().event() << std::endl;
  for (auto jt : *src) {
    
    std::cout << "Jet " << jt.pt()   << " : " << jt.correctedJet("Uncorrected").pt()    <<  " " << jt.eta() << " " << jt.bDiscriminator("combinedSecondaryVertexBJetTags") << std::endl;
  }  
  ...
  {
    edm::LogError("GenericNtupleMakerSNU8TeVPAT") << "Failed to get " << nFailure << " items";
...
void GenericNtupleMakerSNU8TeVPAT::endLuminosityBlock(const edm::LuminosityBlock& lumi, const edm::EventSetup& eventSetup)
{
...
DEFINE_FWK_MODULE(GenericNtupleMakerSNU8TeVPAT);
